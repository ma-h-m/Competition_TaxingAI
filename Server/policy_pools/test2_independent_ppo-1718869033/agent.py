...
from .client import initial_communicate_with_server, push_folder, fetch_random_models, USER_ID
...
def save_train_history(path, history):
    if not os.path.exists(str(path) + '/train_history.csv'):
        with open(str(path) + '/train_history.csv', 'w') as f:
            f.writelines('user_id, epoch, learning_rate\n')
    with open(str(path) + '/train_history.csv', 'a') as f:
        f.writelines(str(history['user_id']) + ',' + str(history['epoch']) + ',' + str(history['learning_rate']) + '\n')
...
class agent:
    def __init__(self,  envs, args, algo_name = "independent_ppo", house_net_path = None, gov_net_path = None, test = False):
        ...
        #     reinit=True
        # )
    def _save_train_history(self, epoch, current_lr):
        history = {"user_id": USER_ID, "epoch": epoch, "learning_rate": current_lr}
        save_train_history(self.model_path, history)
    def _remove_last_train_history(self):
        with open(str(self.model_path) + '/train_history.csv', 'r') as f:
            lines = f.readlines()
        with open(str(self.model_path) + '/train_history.csv', 'w') as f:
            for line in lines[:-1]:
                f.write(line)
    def _get_external_epoch(self):
        if not os.path.exists(str(self.model_path) + '/train_history.csv'):
            return 0
        history_rec = pd.read_csv(self.model_path + '/train_history.csv')
        return history_rec['epoch'].sum()
...
# could be eigher models from server or models from short/long term policy pool
    ...
    def learn(self):
        ...
        house_rew = []
        epochs = []
        external_epochs = self._get_external_epoch()

        # for update in range(self.args.n_epochs):
        for update in range(self.args.local_training_epoch):
            mb_obs, mb_rewards, mb_actions, mb_dones, mb_values = [], [], [], [], []
            gov_mb_obs, gov_mb_rewards, gov_mb_actions, gov_mb_dones, gov_mb_values = [], [], [], [], []
            self._adjust_learning_rate(update + external_epochs, self.args.n_epochs)
            for step in range(self.args.epoch_length):
                with torch.no_grad():
                    # get tensors
                    gov_values, gov_pis = self.gov_net(self._get_tensor_inputs(global_obs))
        ...
        # training with external models
            ...
            if update % self.args.display_interval == 0:
                ...
                if update % self.args.upload_freq == 0:
                    self._save_train_history(now_step, self.args.p_lr)
                    push_folder(os.path.dirname(str(self.model_path)), user_id=USER_ID, model_id=self.identifier, algo_name=self.algo_name, epoch=update + external_epochs)
                    self._remove_last_train_history()
                # save the model
    ...
    def _update_gov_network(self, house_obs, house_actions, house_returns, house_advantages, gov_obs, gov_actions, gov_returns, gov_advantages):
        inds = np.arange(house_obs.shape[0])
        nbatch_train = self.args.batch_size
        for _ in range(self.args.update_epoch):
            np.random.shuffle(inds)

...
class Actor(nn.Module):
    def __init__(self, input_size, output_size, hidden_size=128, step=5):
        ...
        self.split = 4
        self.fc1 = nn.Linear(self.step, self.split)
        self.fc2 = nn.Linear(self.split * self.one_step + self.one_step, hidden_size)
...
class Critic(nn.Module):
    def __init__(self, input_size, hidden_size=128, step=5):
        ...
        self.split = 4
        self.fc1 = nn.Linear(self.step, self.split)
        self.fc2 = nn.Linear(self.split * self.one_step + self.one_step + 13, hidden_size)
    ...
    def forward(self, state, action):
        ...
        x = F.leaky_relu(self.fc2(x))
        x = F.dropout(x, p=0.5)
        x1 = F.leaky_relu(self.fc3_1(x))
        x1 = F.dropout(x1, p=0.5)
        q_value_1 = self.q_out_1(x1)
        x2 = F.leaky_relu(self.fc3_2(x))

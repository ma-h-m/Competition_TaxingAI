    ...
    def learn(self):
        ...
        for epoch in range(self.args.n_epochs):
                ...
                while load_cnt<=3:
                    ...
                    try:
                        if self.args.role == 'gov':
                            ...
                            self.agents[0].random_flag = True if random.random()<0.2 else False
                            self.agents[1].random_flag = True if random.random()<0.2 else False
                            self.agents[2].random_flag = True if random.random()<0.2 else False
                            self.agents[3].random_flag = True if random.random()<0.2 else False
                        else:
                            if random.random()<0.4:
                                dir_path = './model/gov/'
                                filenames = os.listdir(dir_path)
            ...
            if epoch % self.args.save_interval == 0:
                ...
                if self.args.role == 'gov':
                    if self.agents[4].random_flag == False:
                    ...
                    filenames = os.listdir(dir_path)
                    self.agents[4].actor_network.load_state_dict(torch.load(dir_path+random.choice(filenames), map_location="cuda"))
                    final_layer_params = list(self.agents[4].actor_network.action_out.parameters()) 
                    # 为最后一层的权重添加噪声，这里以高斯噪声为例  
                    mean = 0.0  
                    std_dev = 0.01  # 你可以根据需要调整标准差  
                    noise = torch.randn(final_layer_params[0].size()) * std_dev + mean
                    # 更新权重  
                    with torch.no_grad():  # 不需要计算梯度  
                        final_layer_params[0].copy_(final_layer_params[0] + noise.to('cuda')) 
                    self.agents[4].random_flag = False
                else:
                    if self.agents[0].random_flag == False:
                    ...
                    if self.agents[1].random_flag == False:
                    ...
                    if self.agents[2].random_flag == False:
                    ...
                    if self.agents[3].random_flag == False:
                    ...
                    self.agents[3].actor_network.load_state_dict(torch.load(dir_path+random.choice(filenames), map_location="cuda"))
                    for mi in range(4):
                        final_layer_params = list(self.agents[mi].actor_network.action_out.parameters()) 
                        # 为最后一层的权重添加噪声，这里以高斯噪声为例  
                        mean = 0.0  
                        std_dev = 0.01  # 你可以根据需要调整标准差  
                        noise = torch.randn(final_layer_params[0].size()) * std_dev + mean
                        ...
                        with torch.no_grad():  # 不需要计算梯度  
                            final_layer_params[0].copy_(final_layer_params[0] + noise.to('cuda'))
                        self.agents[mi].random_flag = False

...
class matd3_agent:
    ...
    def learn(self):
        ...
        while not self.file_queue.empty():
            try:
                fname = self.file_queue.get(timeout=3)
            except:
                pass
            ...
            for t_ in range(self.opp_times): # random choose 30 times to estimate
                if self.args.role == 'gov': # load others
                    ...
                    for idx in range(4):
                        try:
                            self.agents[idx].actor_network.load_state_dict(torch.load(dir_path+random.choice(filenames), map_location="cuda"))
                        except:
                            pass
                else:
                    if random.random()<0.4:

...
from population_final import matd3_agent
import os
import torch
import yaml
import argparse
...
if __name__ == '__main__':
    ...
    while True:
        if args.role == 'gov':
            dir_path = './model_final/gov/'
            eval_agents = os.listdir(dir_path)
        else:
            dir_path = './model_final/household/'
            eval_agents = os.listdir(dir_path)
        ...
        del_files = ['./model_final/'+args.role+'/'+file_names[_idx] for _idx in last_idx[:del_num]]

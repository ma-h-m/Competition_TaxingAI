...
class MATD3:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        if agent_id == self.args.n_agents-1:   # government agent
            self.actor_network = Actor(args.prev, args.gov_action_dim, args.channel_sizes, kernel_size=args.kernel_size)
            self.actor_target_network = Actor(args.prev, args.gov_action_dim, args.channel_sizes, kernel_size=args.kernel_size)
        else:  # household agent
            self.actor_network = Actor(args.prev, args.house_action_dim, args.channel_sizes, kernel_size=args.kernel_size)
            self.actor_target_network = Actor(args.prev, args.house_action_dim, args.channel_sizes, kernel_size=args.kernel_size)
        
        self.critic_network = Critic(args.prev, 1, args.channel_sizes, kernel_size=args.kernel_size)
        self.critic_target_network = Critic(args.prev, 1, args.channel_sizes, kernel_size=args.kernel_size)
    ...
    def select_action(self, o, noise_rate, epsilon, curr_id):
        ...
        if np.random.uniform() < epsilon or self.random_flag:
    ...
    def train(self, transitions, other_agents, curr_id):
        ...
        #current_Q1, current_Q2 = self.critic_network(o, u)
        actor_loss = - self.critic_network.Q1(o, u).mean()
        if curr_id == self.args.n_agents - 1:
            ...
            teacher_loss = 0.5 * F.mse_loss(new_gov_actions, gov_actions_teacher)
        else:
        ...
        actor_loss = actor_loss + teacher_loss
